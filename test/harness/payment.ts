...
export function makePayment(options: PaymentOptions = {}): {
  ...
  const payload: PaymentPayload = {
    ...
    // A type-literal `ExactEvmPayload` carries an implicit index signature, so
    // it satisfies `Record<string, unknown>` with no cast.
    payload: evmPayload,
 ...
 * boundary. Returns a `Result`: `err` on anything malformed, never a throw and
 * never a propagated `unknown`. The returned value is built from the fields we
 * validated, so no cast is needed to hand back the typed shape.
 */
export function readExactEvmPayload(payload: PaymentPayload): Result<ExactEvmPayload, Error> {
  ...
  if (
    ...
    return err(new Error("exact-EVM authorization is missing required string fields"));
  }
  return ok({
    signature: sig,
